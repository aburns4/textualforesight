...
python -m torch.distributed.run --nproc_per_node=4 --master_port=$mp train.py \
                                                   ...
                                                   --options run.distributed=True \
                                                             run.world_size=4 \
                                                             run.num_workers=1 \
                                                             datasets.aitw_spotlight_stage2_caption.type="default" \
                                                             datasets.longitudinal_spotlight_stage2_caption.type="final" \
                                                             datasets.motif_spotlight_stage2_caption.type="final" \
